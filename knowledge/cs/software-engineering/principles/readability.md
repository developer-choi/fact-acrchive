---
tags: [readability, refactoring, react, architecture]
---
# Questions

- [What are the problems with traditional `if (isLoading)` loading handling?](#what-are-the-problems-with-traditional-if-isloading-loading-handling)
- [How does Suspense achieve separation of concerns?](#how-does-suspense-achieve-separation-of-concerns)
- [What are the benefits of using the Child.loading pattern?](#what-are-the-benefits-of-using-the-child-loading-pattern)
- [Why is extraction harmful and how does abstraction solve it?](#why-is-extraction-harmful-and-how-does-abstraction-solve-it)
- [What is the difference between imperative and declarative programming?](#what-is-the-difference-between-imperative-and-declarative-programming)

---

# Answers

## What are the problems with traditional `if (isLoading)` loading handling?
...
## How does Suspense achieve separation of concerns?

### Official Answer
**성공 케이스에만 집중해서 UI를 작성하겠다라는 선언**이라고 정의한다.

Suspense를 사용하면 컴포넌트 내부에서는 데이터가 반드시 존재한다고 가정할 수 있다. 이는 코드의 복잡도를 획기적으로 낮춰준다.
...
## What are the benefits of using the Child.loading pattern?

### Official Answer
일반적으로 Suspense를 사용할 때, 부모 컴포넌트가 `fallback={<Spinner />}`를 주입한다. 하지만 부모는 자식 컴포넌트의 크기나 모양(Skeleton)을 정확히 알지 못하는 경우가 많다. 이로 인해 부모 컴포넌트에 자식의 스타일링 정보가 누수(Leak)된다.  
...
## Why is extraction harmful and how does abstraction solve it?
...
* **나쁜 추출:** 하나의 논리적 흐름인 100줄짜리 코드를 단순히 20줄씩 5개의 함수로 나누고, 그 함수들이 서로 강하게 결합(Coupling)되어 있다면, 독자는 5개의 함수를 오가며 머릿속으로 조립해야 한다. 이는 인지 부하를 폭발시킨다.  
* **좋은 추상화:** 100줄 중 50줄이 "날짜를 포맷팅하고 상대 시간(방금 전, 1시간 전)을 계산하는 로직"이라면, 이를 `formatRelativeTime(date)`라는 함수로 분리한다. 독자는 함수 이름만 보고 내부 구현을 보지 않아도 된다. 이것이 인지 부하를 줄이는 추상화다.
...
"재사용될 것이 확실하지 않다면 분리하지 않는다"는 원칙을 고수하라. 파일이 200줄이 넘더라도, 하나의 완결된 이야기를 담고 있다면 쪼개진 10개의 파일보다 낫다.
